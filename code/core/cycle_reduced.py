...
class ReducedDDE:
    ...
    def integrate(self, a_hist0, t_total: float, dt: float,
                  record_every: int = 1, da_hist0=None):
        """
        a_hist0: (L+1, P) history at t = -tau..0 on the dt grid (L = tau/dt), or
                 (P,) constant history.
        da_hist0: optional derivative of that history, with shape (L+1, P) or
                  (P,) (broadcast over the history grid). Supplying it is required
                  for high-order Hermite interpolation of a non-constant history
                  that was not produced by this integrator.

        If da_hist0 is omitted, the historical inference used before E34 is kept
        for backward compatibility. New code should pass da_hist0 explicitly and
        re-inject both result["hist"] and result["dhist"] when restarting.

        Returns dict(t, a, hist, dhist). The final history and its derivatives use
        the same grid and can be re-injected without reconstructing the past.
        """
        ...
        a_hist0 = np.asarray(a_hist0, dtype=float)
        if a_hist0.ndim == 1:
            if a_hist0.shape != (P,):
                raise ValueError(
                    f"a_hist0 vector must have shape {(P,)}, got {a_hist0.shape}")
            buf[:L + 1] = a_hist0[None, :]
        else:
            if a_hist0.shape != (L + 1, P):
                raise ValueError(
                    f"a_hist0 history must have shape {(L + 1, P)}, "
                    f"got {a_hist0.shape}")
            buf[:L + 1] = a_hist0
        if not np.all(np.isfinite(buf[:L + 1])):
            raise ValueError("a_hist0 contains NaN or infinite values")

        if da_hist0 is None:
            # Historical behavior retained for old call sites. It is exact for an
            # equilibrium history, but cannot infer the derivative of an arbitrary
            # user-supplied history because values before -tau are unavailable.
            for i in range(L + 1):
                j_del = max(i - L, 0)
                dbuf[i] = self.rhs(buf[i], buf[j_del])
        else:
            da_hist0 = np.asarray(da_hist0, dtype=float)
            if da_hist0.ndim == 1:
                if da_hist0.shape != (P,):
                    raise ValueError(
                        f"da_hist0 vector must have shape {(P,)}, "
                        f"got {da_hist0.shape}")
                dbuf[:L + 1] = da_hist0[None, :]
            elif da_hist0.shape == (L + 1, P):
                dbuf[:L + 1] = da_hist0
            else:
                raise ValueError(
                    f"da_hist0 must have shape {(P,)} or {(L + 1, P)}, "
                    f"got {da_hist0.shape}")
            if not np.all(np.isfinite(dbuf[:L + 1])):
                raise ValueError("da_hist0 contains NaN or infinite values")
