...
# Plot c1
plt.plot(tau_list, ana_c1, 'b-o', label=r'$\lambda_c^{(1)}$ (Analytique)')
plt.plot(tau_list, mcmc_c1, 'b--s', label=r'$\lambda_c^{(1)}$ (MCMC $T=0.05$)')

# Plot c2
plt.plot(tau_list, ana_c2, 'r-o', label=r'$\lambda_c^{(2)}$ (Analytique)')
plt.plot(tau_list, mcmc_c2, 'r--s', label=r'$\lambda_c^{(2)}$ (MCMC $T=0.05$)')

plt.title(r"Valeurs critiques vs $\tau$ ($\alpha=0.05, N=3000$)", fontsize=14)
...
plt.tight_layout()
plt.savefig(str(CHICAGO / "3_numerics/results/_scratch/critical_lambda_vs_tau_a0.05_N3000.png"), dpi=150)
